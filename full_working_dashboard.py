...
# Default high-potential tickers
default_tickers = [
    ...
    for ticker in tickers_list:
        try:
            ...
            try:
                ...
                float_shares = 50000000   # Default 50M
...
if st.sidebar.button("🚀 Run QuantScore Analysis", type="primary") or auto_refresh:
            ...
            if df.empty:
                ...
                if len(final_df) > 0:
                    ...
                    for i, (idx, row) in enumerate(final_df.head(3).iterrows()):
                        color = "success" if i == 0 else "info"
                        if i == 0:
                            st.success(f"🥇 **#{idx}: {row['Ticker']}** - QuantScore: {row['QuantScore']} | Price: {row['Price']} | Change: {row['Change%']}")
                        elif i == 1:
                            st.info(f"🥈 **#{idx}: {row['Ticker']}** - QuantScore: {row['QuantScore']} | Price: {row['Price']} | Change: {row['Change%']}")
                        else:
                            st.info(f"🥉 **#{idx}: {row['Ticker']}** - QuantScore: {row['QuantScore']} | Price: {row['Price']} | Change: {row['Change%']}")
    ...
    with col1:
        st.markdown("""
        ### 🎯 What This Does:
        - **Ranks stocks** by breakout potential using proprietary formula
        - **Real-time analysis** with Yahoo Finance data
        - **Quality filters** for high-momentum picks
        - **Professional analytics** with CSV export
        """)
    ...
    with col2:
        ...
        3. Review ranked results table
        4. Export your top picks as CSV
        """)
